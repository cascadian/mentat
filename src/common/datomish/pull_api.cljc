(ns datomish.pull-api
  ...
  (:require
    [datomish.db :as ddb]
    [datomish.datom :as datom]
    [datomish.schema :as dschema]
...
(defn- pull-attr-datoms
  ...
  (let [attr-key (ddb/ident db attr-key)
        attr (ddb/ident db attr)
        limit (get opts :limit +default-limit+)
        found (not-empty
                (cond->> datoms
                         limit (into [] (take limit))))]
    (if found
      (let [attrid     (ddb/entid db attr)
            schema     (ddb/schema db)
            ref?       (dschema/ref? schema attrid)
            component? (and ref? (dschema/component? schema attrid))
            multi?     (if forward? (dschema/multival? schema attrid) (not component?))
        ...
        (let [attr (:attr opts)
              ...
              results (if forward?
                        ...
                        (<? (ddb/<?q db '[:find ?e ?a ?v ?tx
                                     ...
                                     {:inputs {:v eid
                                               :a (ddb/entid db attr)}})))
              results (map tuple->Datom results)]
...
(defn- <pull-wildcard-expand
  ...
  (go-pair
    (let [datoms (group-by (fn [^Datom d] (.-a d)) (<? (<datoms-by-eid db eid)))
...
(defn- <pull-pattern
  ...
  (go-pair
    (case (:state (first frames))
      :expand (<? (<pull-pattern db (pull-expand-frame db frames)))
      :expand-rev (<? (<pull-pattern db (pull-expand-reverse-frame db frames)))
      :pattern (<? (<pull-pattern db (<? (<pull-pattern-frame db frames))))
      :recursion (<? (<pull-pattern db (pull-recursion-frame db frames)))
...
(defn <pull-spec
  ...
  (let [eids (into [] (map #(ddb/entid db %)) eids)]
